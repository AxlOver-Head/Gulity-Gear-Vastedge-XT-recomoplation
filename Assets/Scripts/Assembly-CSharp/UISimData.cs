...
public class UISimData : MonoBehaviour
{
	public enum MODE
	{
		INIT = 0,
	...
	private UISimDataData m_DataData;

	private UISimDataBonus m_DataBonus;

	private UISimDataRireki[] m_DataRireki;

	private GameObject m_ScrollPanel;

	private UISprite m_Arrow;

	private UISimDataSlump m_DataSlump;
	...
	private void Start()
	{
		...
		m_IsDrag = false;
		m_IsFlick = false;
		m_BackButton = GameObject.Find(base.gameObject.name + "/Camera/Anchor1_Top/Panel/Button_Back");
		...
		m_Arrow_L = GameObject.Find(base.gameObject.name + "/Camera/Anchor3_Bottom/Panel/Arrow_L");
		...
		m_Arrow_R = GameObject.Find(base.gameObject.name + "/Camera/Anchor3_Bottom/Panel/Arrow_R");
		...
		setInput(false);
		m_Rireki = GameObject.Find(base.gameObject.name + "/Camera/Anchor1_Top/Panel_3_Rireki/ScrollPanel/Rireki");
		m_ScrollPanel = GameObject.Find(base.gameObject.name + "/Camera/Anchor1_Top/Panel_3_Rireki/ScrollPanel");
		m_Arrow = GameObject.Find(base.gameObject.name + "/Camera/Anchor1_Top/Panel_3_Rireki/Arrow_2").GetComponent<UISprite>();
		m_Panels = new GameObject[PAGE_MAX];
		m_Panels[0] = GameObject.Find(base.gameObject.name + "/Camera/Anchor2_Center/Panel_1_Data");
		m_Panels[1] = GameObject.Find(base.gameObject.name + "/Camera/Anchor2_Center/Panel_2_Bonus");
		m_Panels[2] = GameObject.Find(base.gameObject.name + "/Camera/Anchor1_Top/Panel_3_Rireki");
		m_Panels[3] = GameObject.Find(base.gameObject.name + "/Camera/Anchor2_Center/Panel_4_Slump");
		m_Titles = new UISprite[PAGE_MAX];
		m_Pages = new UISprite[PAGE_MAX];
		for (int i = 0; i < PAGE_MAX; i++)
		{
			m_Titles[i] = GameObject.Find(base.gameObject.name + "/Camera/Anchor1_Top/Panel/Title" + (i + 1)).GetComponent<UISprite>();
			m_Pages[i] = GameObject.Find(base.gameObject.name + "/Camera/Anchor3_Bottom/Panel/Page_" + (i + 1)).GetComponent<UISprite>();
		}
		findData();
		initData();
		findBonus();
	...
	{
		m_DataData = new UISimDataData(this);
	}

	private void findBonus()
	{
		m_DataBonus = new UISimDataBonus(this);
	}

	private void findRireki()
	{
		m_DataRireki = new UISimDataRireki[RIREKI_NUM];
		for (int i = 0; i < RIREKI_NUM; i++)
		{
			m_DataRireki[i] = new UISimDataRireki(i, this);
		}
	}

	private void findSlump()
	{
		m_DataSlump = new UISimDataSlump(this);
	}
	...
	{
		m_DataSlump.setData(MainGameScene.slotGameMan.m_SlumpDataSim);
	}
	...
	private void Update()
	{
		...
		case MODE.END:
			...
			else if (SGLG.isFadeOutComplete())
			{
				setVisibleMain(true);
