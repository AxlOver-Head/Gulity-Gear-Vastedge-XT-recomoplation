...
public class UIMenuSetting : MonoBehaviour
{
	...
	{
		m_PushOrder = PlayUserData.saveData.m_configOshi;
		m_ReelSpeed = PlayUserData.saveData.m_configReelSpeed;
		m_isReelWait = PlayUserData.saveData.m_configReelWait == 1;
		m_isReelBlur = PlayUserData.saveData.m_configReelBlur == 1;
		m_isDataCompact = PlayUserData.saveData.m_configDataSize == 1;
	}

	private void Start()
	{
		base.gameObject.name = "Menu_Setting";
		m_ClickButtonID = -1;
		m_IsClick = false;
		for (int i = 0; i < 2; i++)
		{
			m_ObjPushOrder[i] = GameObject.Find("Menu_Setting/Camera/Anchor1/Panel2/Panel_OshiJun/Button_" + (i + 1));
		}
		for (int j = 0; j < 3; j++)
		{
			m_ObjReelSpeed[j] = GameObject.Find("Menu_Setting/Camera/Anchor1/Panel2/Panel_ReelSpeed/Button_" + (j + 1));
		}
		for (int k = 0; k < 2; k++)
		{
			m_ObjReelWait[k] = GameObject.Find("Menu_Setting/Camera/Anchor1/Panel2/Panel_ReelWait/Button_" + (k + 1));
			m_ObjReelBlur[k] = GameObject.Find("Menu_Setting/Camera/Anchor1/Panel2/Panel_ReelBlur/Button_" + (k + 1));
			m_ObjDataCompact[k] = GameObject.Find("Menu_Setting/Camera/Anchor1/Panel2/Panel_DataCompact/Button_" + (k + 1));
		}
		m_Bx_Back = GameObject.Find("Menu_Setting/Camera/Anchor2/Panel/Button_Back").GetComponent<BoxCollider>();
		m_Bx_Help = GameObject.Find("Menu_Setting/Camera/Anchor2/Panel/Button_Help").GetComponent<BoxCollider>();
		m_Bx_Start = GameObject.Find("Menu_Setting/Camera/Anchor1/Panel2/Button_Start").GetComponent<BoxCollider>();
	...
	{
		PlayUserData.saveData.m_configOshi = (byte)m_PushOrder;
		PlayUserData.saveData.m_configReelSpeed = (byte)m_ReelSpeed;
		PlayUserData.saveData.m_configReelWait = (byte)(m_isReelWait ? 1u : 0u);
		PlayUserData.saveData.m_configReelBlur = (byte)(m_isReelBlur ? 1u : 0u);
		PlayUserData.saveData.m_configDataSize = (byte)(m_isDataCompact ? 1u : 0u);
