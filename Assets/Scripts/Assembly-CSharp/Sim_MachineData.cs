...
public class Sim_MachineData : MonoBehaviour
{
	...
	public static Sim_MachineSaveData saveData = new Sim_MachineSaveData();
	...
	private void Awake()
	{
		if (!m_initAwake)
		{
			m_filename = GGLFile.filePath("Sim_MachineData.sav");
			saveData.clear();
			...
			else if (saveData.AppTimer > saveData.SerVerTime)
			{
				saveData.SerVerTime = saveData.AppTimer;
			}
			m_initAwake = true;
		}
		m_bPause = false;
