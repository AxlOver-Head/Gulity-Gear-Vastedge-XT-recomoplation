public class DirectionCommandMovieAddWithLayerAndPosition : DirectionCommandBase
{
	...
	public DirectionCommandMovieAddWithLayerAndPosition(Production p, int playID, int oldID, float layer, bool kill, bool loop, bool add, bool trim, int x, int y, int priority)
		: base(0)
	{
		prod = p;
		m_PlayID = playID;
		...
		m_X = x;
		m_Y = y;
		m_Class = CLASS.ADDMOVIEWITHLAYERANDPOS;
		m_Priority = priority;
