public class DirectionCommandAnimeAdd : DirectionCommandBase
{
	...
	public DirectionCommandAnimeAdd(ProductionAnime p, int playID, int ptnID, int oldID, bool kill, bool loop, bool add, bool trim, int priority)
		: base(0)
	{
		prodAnime = p;
		m_PlayID = playID;
	...
	public override void control(int cnt)
	{
		...
		{
			m_X = prodAnime.GetX(num);
			m_Y = prodAnime.GetY(num);
