public class Direction5CST : DirectionBase
{
	public Direction5CST(DirectionManager _mng, byte _high, byte _low, byte _mode, DirectionManager.DIR _dir)
	...
	public static void start_0_1(DirectionEvent _evt, DirectionManager _mng)
	{
		...
		_evt.eventPlayImage(DirectionManager.IMAGE.cst6001_b_c_a01, false, true, 128, 72, 200, 0);
		_evt.eventPlayMovie(DirectionManager.MOVIE.chk001_1, false, false, false, false, 0, 0, 199, 0);
		_evt.eventPlayMovie(DirectionManager.MOVIE.chk001_1_add, false, true, false, false, 0, 0, 197, 0);
		restartBGM();
	}

	public static void restartBGM()
	{
		if (SGLG.sound.getBGMID() == 6)
		{
		...
		{
			SGLG.sound.PlaySound(6, Sound.CH.BGM, true);
			SGLG.sound.SetVolume(Sound.CH.BGM, 1f);
		}
	}
