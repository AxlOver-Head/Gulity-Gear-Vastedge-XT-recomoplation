public class Direction33DST : DirectionBase
{
	public Direction33DST(DirectionManager _mng, byte _high, byte _low, byte _mode, DirectionManager.DIR _dir)
	...
	public static void start_0_1(DirectionEvent _evt, DirectionManager _mng)
	{
		...
		_evt.eventPlayMovie(DirectionManager.MOVIE.dst001, false, true, false, false, 0, 0, 190, 0);
		_evt.eventPlaySound(DirectionManager.SOUND.S0199, Sound.CH.SE0, true, 1f, 0);
		restartBGM();
	}

	public static void restartBGM()
	{
		if (SGLG.sound.getBGMID() == 74)
		{
		...
		{
			SGLG.sound.PlaySound(74, Sound.CH.BGM, true);
			SGLG.sound.SetVolume(Sound.CH.BGM, 1f);
		}
	}
